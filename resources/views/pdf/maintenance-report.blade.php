...
<body>
    @php
        // Setup data untuk setiap section
        $asset = $report->asset;
        ...
        $sections->push([
            ...
            'attachments' => $report->attachments->filter(fn ($a) => str_ends_with($a->slot_key, '_2') === false)->values(),
            'tanggal_pelaksanaan' => $report->tanggal_pelaksanaan,
        ]);
        
        // Bagian tambahan (dari items)
        ...
        // Styles untuk kartu yang independen di halaman yang sama
    @endphp
    
    <style>
        .maintenance-card { page-break-inside: avoid; }
        @if ($multi)
            .maintenance-card:not(:last-child) { margin-bottom: 8mm; }
            .card-spacer { height: 6mm; background: #f4f4f4; }
        @endif
    </style>

    @foreach ($sections as $section)
        <div class="maintenance-card">
            <table class="doc-meta" style="width: 100%; margin-bottom: 2mm;">
                <tr>
            ...
            <table class="paper-form">
                ...
                <tr class="center">
                    <td>Tanggal</td>
                    <td colspan="2">{{ $section['tanggal_pelaksanaan']?->format('d-m-Y') ?? now()->format('d-m-Y') }}</td>
